...
<input type="image"  src="../imagenes/boton-buscar.png"/>
        ...
        <table width="800" align="center"  bgcolor="#FFFFFF">
          <tr>
            ...
            <td width="92" align="center" background="../imagenes/barra-btn-fdo.png" class="campo">Modificaciones</td>
...
<?PHP
 ...
 #se seleccion los campos de la pag consultas
		  ...
		  SELECT 
			...
			where [fecha_db] between '$_POST[f_inicio]' and '$_POST[f_fin]'
			and CONVERT(VARCHAR,categoria_db )='$_POST[id_tipo_operativo]'
			and CONVERT(VARCHAR,subcategoria_db )='$_POST[id_autoridad]'
			
			 ORDER BY CONVERT(VARCHAR, nombre_db) ASC
			
		  ";
		  
		  #CONVERT(VARCHAR, nombre_db)  se convierte  a var char para poder hacer la modificaciions ya que como es tipo text
		  # no lo detecta y se tiene que convertir
...
while ($row=mssql_fetch_array($resultados))
          ...
          <tr  id="tabla-color"  style="  " class="<? if ($bgcolor==false) echo 'FONDO_TABLA_FILA_2'; else 'FONDO_TABLA_FILA_1'; ?>">
...
<!--Para crear el link para mandar modificar los datos
<td align="center" class="contenido"><a target="_blank" href="http://10.10.1.31/Practicas/ulises/alta_ejemplo.php">MOD user</a></td>
-->

 
<td align="center" class="contenido"><a  target="_blank" href="http://10.10.1.31/Practicas/ulises/modificaciones.php?ref=<?PHP echo $row['id_user'];?>">Editar Inf.</a></td>


<!--?ref= echo $row['id_user'];?>"  es para enviar el id del usuario al cual fue seleccionado en el link  recibiendolo con un get en la otra pag-->
       ...
       <!--este no es necesario, solo es para pruebas-->
        <input name="result" type="text" size="10" 
        value="<?php echo "$_POST[id_tipo_operativo]";?>" >
